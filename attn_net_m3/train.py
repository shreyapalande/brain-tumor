"""
train.py -- Baseline training for attn_net_m3

Trains an Attention U-Net with decoder CBAM (dec4/3/2/1) from random
initialisation.
...
-----------------
  ...
  Epochs    : 150  (early-stop with patience=20 validation checks)
  Batch     : 2 patches per step, 400 steps per epoch
  Loss      : 0.3 * CE + 0.5 * Dice + 0.2 * ET_Dice

Identical training procedure to attn_net_m1/m2 -- only the model
architecture differs (CBAM at decoder vs. encoder vs. bottleneck).

Usage
-----
    python -m github.attn_net_m3.train \\
        [--data_root PATH] [--save_dir PATH]
"""
...
from ..attn_net_m2.losses  import BaselineCriterion
...
def run_training(cfg: dict) -> None:
    ...
    log("=" * 80)
    log("Attention U-Net M3 (Decoder CBAM: dec4/3/2/1) -- Baseline Training")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train Attention U-Net M3 (Decoder CBAM).")
