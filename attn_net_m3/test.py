"""
test.py -- Evaluation of attn_net_m3 on the BraTS test split

Loads the trained decoder-CBAM checkpoint and reports the full suite of
BraTS segmentation metrics:
...
Usage
-----
    python -m github.attn_net_m3.test \\
        [--model_path PATH] [--data_root PATH] \\
...
from ..attn_net_m1.metrics import (brats_wt_tc_et, iou_binary,
...
# Helpers
# ---------------------------------------------------------------------------
def _pick_tumour_slice(gt_dhw: np.ndarray) -> int:
    """Return axial index of the median tumour-containing slice."""
    D            = gt_dhw.shape[0]
...
def _save_visualisation(flair: np.ndarray, gt: np.ndarray,
                         pred: np.ndarray, pid: str,
                         save_dir: str) -> None:
    """Save 3-panel axial visualisation (FLAIR | GT | Prediction)."""
    z = _pick_tumour_slice(gt)
...
def run_test(cfg: dict) -> None:
    ...
    with torch.no_grad():
        for pid in tqdm(test_patients, desc="Testing"):
            ...
            if visualized < MAX_VIZ:
                _save_visualisation(flair_np, seg, pred, pid, cfg["save_dir"])
                visualized += 1
    ...
    lines = [
        "=" * 60,
        "FINAL TEST RESULTS -- attn_net_m3",
        "(Attention U-Net, Decoder CBAM: dec4/dec3/dec2/dec1)",
        "=" * 60,
        f"ET Dice        : {m[0]:.4f}",
        f"TC Dice        : {m[1]:.4f}",
        f"WT Dice        : {m[2]:.4f}",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate attn_net_m3 on BraTS test split.")
