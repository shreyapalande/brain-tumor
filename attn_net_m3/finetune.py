"""
finetune.py -- Boundary-aware fine-tuning for attn_net_m3

Loads the pre-trained attn_net_m3 (decoder CBAM) checkpoint and
fine-tunes it with multi-level boundary-aware supervision.
...
Loss
----
  total = seg_loss + lambda_disc * mean(L_disc_dec4, L_disc_dec3, L_disc_dec2)

  seg_loss = 0.25 * CE_boundary  (boundary_weight=6.0)
           + 0.40 * Dice
           + 0.15 * ET_Dice
           + 0.20 * TC_Dice

  L_disc: SA discriminability hinge loss applied at decoder levels
          dec4 (D/8), dec3 (D/4), dec2 (D/2).
          dec1 (full resolution D x H x W) is intentionally excluded --
          gradients are too noisy at 128^3 resolution.
          The attn_dict keys are aliased as "cbam4", "cbam3", "cbam2"
          to match the MultiLevelBoundaryAwareCriterion from attn_net_m2.

Usage
-----
    python -m github.attn_net_m3.finetune \\
        [--checkpoint_path PATH] [--save_dir PATH] [--data_root PATH]
"""
...
from ..attn_net_m2.losses  import MultiLevelBoundaryAwareCriterion
from ..attn_net_m1.dataset import Brain3DDataset, Brain3DValDataset
from ..attn_net_m1.metrics import dice_per_class
...
def run_finetuning(cfg: dict) -> None:
    ...
    log("=" * 80)
    log("attn_net_m3 -- Boundary-Aware Fine-Tuning (Decoder CBAM)")
    ...
    log(f"CBAM levels supervised: {cfg['cbam_levels']} "
        f"(cbam4=dec4/D8, cbam3=dec3/D4, cbam2=dec2/D2)")
    log(f"dec1 (D x H x W) NOT supervised -- gradient too noisy at full res")
    ...
    scaler    = GradScaler()
    criterion = MultiLevelBoundaryAwareCriterion(
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for i, batch in enumerate(pbar):
            ...
            with autocast():
                ...
                logits   = attn_out["out"]
                # Keys aliased as cbam4/3/2 from dec4/3/2
                attn_dict = {
                    "cbam4": attn_out["cbam4"],
                    "cbam3": attn_out["cbam3"],
                    "cbam2": attn_out["cbam2"],
                }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Boundary fine-tuning for attn_net_m3 (Decoder CBAM).")
    ...
    if args.checkpoint_path: cfg["checkpoint_path"] = args.checkpoint_path
