"""
finetune.py -- Boundary-aware fine-tuning for attn_net_m2

This script loads a pretrained attn_net_m2 checkpoint and fine-tunes
it with boundary-aware loss applied to multiple CBAM levels.

Fine-tuning schedule:
  - AdamW, lr=5e-6, weight_decay=1e-4
  - ReduceLROnPlateau (patience=8, factor=0.5, min_lr=1e-7)
  - 80 epochs (early stop after 20 val checks)
  - Batch: 2 patches, 400 patches/epoch

Loss details:
  total = seg_loss + lambda_disc * mean(L_disc_enc2, L_disc_enc3, L_disc_enc4)
  seg_loss = boundary-weighted CE + Dice + ET Dice + TC Dice
  L_disc applies hinge SA discriminability loss at enc2/enc3/enc4.

Usage:
    python -m github.attn_net_m2.finetune \\
...
from .losses  import MultiLevelBoundaryAwareCriterion
...
def run_finetuning(cfg: dict) -> None:
    ...
    log("=" * 80)
    log("attn_net_m2 -- Boundary-Aware Fine-Tuning (Encoder CBAM)")
    ...
    log(f"Resolutions:  enc2=D/2, enc3=D/4, enc4=D/8")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Boundary fine-tuning for attn_net_m2 (Encoder CBAM).")
