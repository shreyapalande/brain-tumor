"""
test.py -- Evaluation of attn_net_m2 on the BraTS test split

Loads the trained encoder-CBAM checkpoint and reports the full suite of
...
Usage
-----
    python -m github.attn_net_m2.test \\
...
def run_test(cfg: dict) -> None:
    ...
    lines = [
        "=" * 60,
        "FINAL TEST RESULTS -- attn_net_m2",
        "(Attention U-Net, Encoder CBAM: enc2/enc3/enc4)",
        "=" * 60,
        f"ET Dice        : {m[0]:.4f}",
        f"TC Dice        : {m[1]:.4f}",
        f"WT Dice        : {m[2]:.4f}",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate attn_net_m2 on BraTS test split.")
